...
# 3.Dono Backend aur Worker ke liye Celery aur Gunicorn saath mein chalayenge
echo "Starting Celery Worker and Gunicorn Server..."

# Celery ko background me chalane ke liya  (& lagana zaroori hai)
python -m celery -A config worker --loglevel=info &

# Gunicorn ko main process bana kar chalayenge (Isse container zinda rahega aur Port detect hoga)
